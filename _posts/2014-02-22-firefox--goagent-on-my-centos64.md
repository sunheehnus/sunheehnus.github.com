...
1. GoAgent需要使用[python2.7](http://python.org/downloads/)，CentOS自带的python2.6安装完毕会存在错误，需要手动编译安  
    * tar vxzf Python-2.7.x.tgz
    * cd Python-2.7.x
...
2. 安装[gevent](https://pypi.python.org/pypi/gevent#downloads)
    * tar vxzf gevent-1.0.tar.gz
    * cd gevent-1.0/
    * sudo ./setup.py install
3. 安装[greenlet](http://pypi.python.org/pypi/greenlet)
...
6. 导入证书
    * Firefox->Preferences->Advances->Certificates->View Certificates->Authorities->Import(local/ca.crt)
    * aggree everything
7. 设置浏览器代理地址为127.0.0.1:8087(应该使用AutoProxy插件代替)
    * 安装插件AutoProxy
